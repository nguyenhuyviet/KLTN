...
@Component({
  selector: 'ngx-list-process-done',
  templateUrl: './list-process-done.component.html',
  styleUrls: ['./list-process-done.component.scss']
})
export class ListProcessDoneComponent implements OnInit {
  ...
  getProcess() {
    this.isLoading = true;
    this.processExeSV.getProcessRelated(this.paging).subscribe(data => {
  ...
  setFilter(type) {
    if (!this.paging.ExtraCondition) {
      this.paging.ExtraCondition = {
        ProcessRelatedType: 1
      };

    } else if (this.paging.ExtraCondition.ProcessRelatedType == type) {
      this.paging.ExtraCondition.ProcessRelatedType = 0;
    } else {
